...
std::pair<std::string, std::string> Config::parseOptionLine(std::string optionLine)
{
...
std::unordered_map<std::string, std::string> Config::getOptionBlock(std::string blockName)
{
	std::unordered_map<std::string,std::unordered_map<std::string, std::string>>::const_iterator got = options.find(blockName);
	if(got == options.end())
	{
...
std::unordered_map<std::string, std::unordered_map<std::string,std::string>> Config::getAllOptionBlocks()
{
	return options;
}
...
std::ostream & operator << (std::ostream &os, Config &c)
{
	for(auto block:c.getAllOptionBlocks())
	{
		...
		{
			os<<"\t"<<"option name = "<<blockOpt.first<<"\n";
			os<<"\t"<<"option value = "<<blockOpt.second<<"\n";
		}
	}
	return os;
}
