...
//---UserDefine
std::vector<GuideProfile> GuideSrcInfoByKlee;
...
namespace {
  ...
  cl::opt<bool>
  GuidedExecutionTag("guidedExe",
                   cl::init(false),
		   cl::desc("Enable guided symbolic execution"));
  ...
  for (Module::const_global_iterator i = m->global_begin(),
    ...
    if (i->isDeclaration()) {
      ...
      if (size) {
        ...
        if (!addr)
          continue;
          //---UserDefine----igonre unknow global symbols
...
//---UserDefine----record path
void RecordPath(KInstruction *ki){
  const InstructionInfo &ins = *ki->info;
  std::stringstream t_sstream;
  if(ins.line!=0)
    t_sstream<<ins.file<<'\t'<<ins.line<<std::endl;
  std::string filename="pathrecord.txt";
  std::ofstream fileout(filename.c_str(),std::ios::app);
...
void Executor::run(ExecutionState &initialState) {
  if(GuidedExecutionTag)
  {
    const char* file_name = "./GuideSrc.txt";
    GuideSrcInfoByKlee = GuideProfile::fileread(file_name);
  }
