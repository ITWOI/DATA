...
bool ListContain(ExecutionState* state){
  KInstruction *ktrue = (*state).pc;
  const InstructionInfo &i = *ktrue->info;
  for(unsigned int j=0;j<GuideSrcInfoByKlee.size();j++)
    if((i.line == GuideSrcInfoByKlee.at(j).Executionline) && filepathCanonical(GuideSrcInfoByKlee.at(j).file_name).compare(filepathCanonical(i.file))==0){
    {
      return true;
    }
  }
...
//---UserDef---delete the useless state---------- 
void Searcher::GuideStateExecution(std::set<ExecutionState*> &removedStates,ExecutionState* firstbranch,ExecutionState* secondbranch){
  if(firstbranch != NULL && secondbranch != NULL){
    if(ListContain(firstbranch) && !ListContain(secondbranch) )
    {
      removedStates.insert(secondbranch);
    }
    else if( !ListContain(firstbranch) && ListContain(secondbranch) )
    {
      removedStates.insert(firstbranch);
    }
  }
}
