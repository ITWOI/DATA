...
class ASTFunctionLoad : public ASTConsumer, public RecursiveASTVisitor<ASTFunctionLoad> {    
    ...
    public:
        ...
        bool TraverseDecl(Decl *D) {
            if (!D) return true;
            bool rval = true;
            if (D->getASTContext().getSourceManager().isInMainFile(D->getLocation()) || D->getKind() == Decl::TranslationUnit) {
...
std::unique_ptr<ASTUnit> loadFromASTFile(std::string AST) {
    ...
    return std::unique_ptr<ASTUnit>(ASTUnit::LoadFromASTFile(AST, Diags, FileSystemOpts));
}
